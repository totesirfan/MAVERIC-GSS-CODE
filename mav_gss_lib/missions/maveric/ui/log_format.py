"""
mav_gss_lib.missions.maveric.ui.log_format -- MAVERIC Logging Format Helpers
...
def _format_gnc_register_lines(reg_name: str, value, unit: str = "") -> list[str]:
    ...
    `value` is the structured payload the mission extractor attached to
    the telemetry fragment. `unit` travels on the fragment too, populated
    by the mission extractors from the semantic decoder's unit table —
    no catalog lookup at render time.
    """
