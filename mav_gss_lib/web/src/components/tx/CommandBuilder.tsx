...
import type { CommandSchema, CommandDef, GssConfig } from '@/lib/types'
...
export function CommandBuilder({ config, onQueue, onClose }: CommandBuilderProps) {
  ...
  // Focus first arg when command selected
  useEffect(() => {
    if (selectedCmd) setTimeout(() => firstArgRef.current?.focus(), 50)
  }, [selectedCmd])

  // Nodes from config, exclude GS and NONE
  const nodes = useMemo(() => {
    if (!config?.nodes) return []
    return Object.entries(config.nodes)
      .map(([id, name]) => ({ id: Number(id), name }))
      .filter(n => n.name !== 'GS' && n.name !== 'NONE')
  }, [config])

  // Commands filtered by selected node + search
  ...
  function handleQueue() {
    ...
    onQueue(selectedCmd, argValues, destNode, echo || 'NONE', cmdDef?.ptype)
    setArgValues({})
    setSelectedCmd(null)
    setSearch('')
  }
  ...
  return (
    ...
    >
      ...
      <div className="p-2 space-y-2">
        {/* 1. Destination node — always visible */}
        <div>
        ...
        {/* 2. Command picker — visible after node selected */}
        {destNode && (
          <div>
        ...
        {/* 3. Args — visible after command selected */}
        {selectedCmd && (
          <div>
            {txArgs.length > 0 ? (
              <>
        ...
        {/* Preview + Queue — visible when ready */}
