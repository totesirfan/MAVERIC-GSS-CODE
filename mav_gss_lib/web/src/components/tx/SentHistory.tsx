                ...
                <ContextMenuRoot key={item.n}>
                  <ContextMenuTrigger>
                    <div className="flex items-center gap-2 px-3 py-1.5 text-xs font-mono hover:bg-white/[0.03]">
                      ...
                      <span className={`${col.node} shrink-0 truncate`} style={{ color: colors.label }}>{item.dest}</span>
                  ...
                  <ContextMenuContent>
                    <ContextMenuItem icon={ClipboardCopy} onSelect={() => navigator.clipboard.writeText(`${item.cmd} ${item.args}`.trim())}>
                      Copy Command
                    </ContextMenuItem>
                    {onRequeue && (
