...
> **This document has been retired.** The pre-platform `MissionAdapter` boundary no
> longer exists. Missions are added as `MissionSpec` packages.
> Until a full rewrite of this guide lands, use the following sources directly:
>
> - **Mission boundary contract** — `mav_gss_lib/platform/contract/mission.py`
>   (`MissionSpec`, `MissionContext`, `MissionConfigSpec`, `UiOps`, `HttpOps`).
> - **Capability protocols** — `mav_gss_lib/platform/contract/packets.py` (`PacketOps`),
>   `mav_gss_lib/platform/contract/commands.py` (`CommandOps`),
>   `mav_gss_lib/platform/contract/telemetry.py` (`TelemetryOps`),
>   `mav_gss_lib/platform/contract/events.py` (`EventOps`).
