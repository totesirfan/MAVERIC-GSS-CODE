...
> **This document has been retired.** It described the pre-v2 adapter-era
> runtime. The current startup path uses `PlatformRuntimeV2` and
...
>   then calls `mav_gss_lib.web_runtime.app.create_app()`.
> - **Runtime container** — `mav_gss_lib/web_runtime/state.py::WebRuntime`
>   owns split config (`platform_cfg`, `mission_id`, `mission_cfg`), the
>   `PlatformRuntimeV2`, `MissionSpec`, RX/TX services, and session/update/preflight state.
> - **Platform v2 API** — `mav_gss_lib/platform/` (contracts in
>   `mission_api.py`, `commands.py`, `packets.py`, `rendering.py`,
>   `telemetry.py`, `events.py`; pipelines in `*_pipeline.py`;
>   `runtime.py::PlatformRuntimeV2` is the production constructor).
...
>   `tests/test_api_config_get_contract.py` enforce the v2 boundary
>   invariants (no `runtime.cfg` reads, no `mav_gss_lib.protocols.*`
>   imports under `web_runtime/`, no `runtime.csp` / `runtime.ax25`
>   access, etc).
